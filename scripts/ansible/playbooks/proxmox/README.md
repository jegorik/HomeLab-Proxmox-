...
| `ignition_config_escaped` | ✅ | - | Escaped Ignition JSON |
| `start_vm_after_ignition` | ❌ | `true` | Start VM after config |
| `wait_for_ssh` | ❌ | `true` | Wait for SSH access |
...
```text
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│  Read Ignition  │────▶│  qm set -args   │────▶│  qm start VM    │
│  from vars file │     │  (fw_cfg)       │     │                 │
└─────────────────┘     └─────────────────┘     └─────────────────┘
                                                        │
                                                        ▼
                                               ┌─────────────────┐
